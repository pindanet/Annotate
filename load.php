...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $fd = htmlspecialchars(stripslashes(trim($_POST["file"])));
...
// https://encrypt-online.com/encrypt-and-decrypt-in-php
  ...
  $blob = openssl_decrypt(gzuncompress(file_get_contents($fd)), $encrypt_method, $passphrase, $options = OPENSSL_RAW_DATA, $iv);
} else {
  $blob = file_get_contents($fd);
}
echo $blob;
?>
